package scala.reflect.internal.prettyprint
...
trait Printers {
  ...
  class TreePrinter(out: PrintWriter) {
    ...
    // typically there's no point to print mirrors inside the compiler, as there is only one mirror there
    ...
    def printModifiers(tree: Tree, mods: Modifiers): Unit = {
      printFlags(if (tree.symbol == NoSymbol) mods.flags else tree.symbol.flags, "" + (if (tree.symbol == NoSymbol) {
        mods.privateWithin
    ...
    def printFlags(flags: Long, privateWithin: String) = {
      ...
      val s = {
        ...
        val m_flagsToString :: Nil = flagModule.getClass.getMethods
          .filter(m => m.getName == "flagsToString" && m.getParameterTypes().length == 2).toList
  ...
  class CodePrinter(out: PrintWriter, printRootPkg: Boolean) extends TreePrinter(out) {
    ...
    protected def printedName(name: Name, decoded: Boolean = true) = {
      ...
      def addBackquotes(s: String) = {
        if (decoded && (decName.exists(ch => brackets.contains(ch) || isWhitespace(ch)) ||
          (name.isOperatorName && decName.exists(isOperatorPart) && decName.exists(isScalaLetter) &&
    ...
    protected def needsParentheses(parent: Tree)
                                  (insideIf: Boolean = true, insideMatch: Boolean = true, insideTry: Boolean = true,
                                   insideAnnotated: Boolean = true, insideBlock: Boolean = true,
                                   insideLabelDef: Boolean = true, insideAssign: Boolean = true) = {
      parent match {
        case _: If => insideIf
    ...
    protected def resolveSelect(t: Tree): String = {
      t match {
        ...
        case Select(qual, name) if (name.isTermName && needsParentheses(qual)(insideLabelDef = false)) ||
          isIntLitWithDecodedOp(qual, name) => s"(${resolveSelect(qual)}).${printedName(name)}"
        case Select(qual, name) if name.isTermName => s"${resolveSelect(qual)}.${printedName(name)}"
        case Select(qual, name) if name.isTypeName => s"${resolveSelect(qual)}#${
          blankForOperatorName(name)
        }%${printedName(name)}"
    ...
    protected def emptyTree(tree: Tree) = {
      tree match {
    ...
    protected def originalTypeTrees(trees: List[Tree]) = {
      trees.filter(!emptyTree(_)) map { case tt: TypeTree => tt.original
      case tree => tree
      }
    }
    ...
    protected def removeDefaultTypesFromList(trees: List[Tree])(classesToRemove: List[Name] = defaultClasses)
    ...
    def printModifiers(mods: Modifiers, primaryCtorParam: Boolean): Unit = {
      def modsAccepted = {
        List(currentTree, currentParent) exists
          (_ map { case _: ClassDef | _: ModuleDef | _: Template | _: PackageDef => true
          case _ => false
          } getOrElse false)
      }
      ...
      if (currentParent.isEmpty || modsAccepted) {
        ...
        List(IMPLICIT, CASE, LAZY, SEALED)
          .foreach { flag => if (mods.hasFlag(flag)) print(s"${mods.flagBitsToString(flag)} ")}
      }
    }
    ...
    def processTreePrinting(tree: Tree): Unit = {
      tree match {
        ...
        case t@Template(parents, self, tbody) =>
          ...
          val printedParents =
            currentParent map { case _: CompoundTypeTree => parents
            case ClassDef(mods, name, _, _) if mods.isCase => removeDefaultTypesFromList(parents)()(
          ...
          val ap: Option[Apply] = primaryCtr match {
            case DefDef(_, _, _, _, _, Block(ctBody, _)) =>
              val earlyDefs = treeInfo.preSuperFields(ctBody) :::
                body.filter { case td: TypeDef => treeInfo.isEarlyDef(td)
                case _ => false
                }
          ...
          val (left, right) = body.filter {
            ...
            case _ => true
          } span { case dd: DefDef => dd.name != nme.CONSTRUCTOR
          case _ => true
          }
        ...
        case Typed(expr, tp) =>
          ...
          tp match {
            ...
            case Annotated(annot, arg) if (expr ne null) && (arg ne null) &&
              expr.equalsStructure(arg) => printTp // remove double arg - 5: 5: @unchecked
        ...
        case Select(qual, name) =>
          def checkRootPackage(tr: Tree): Boolean = {
            (currentParent match {
              //check that Select is not for package def name
  ...
  def showCode(tree: Tree, printTypes: BooleanFlag = None, printIds: BooleanFlag = None,
               printOwners: BooleanFlag = None, printPositions: BooleanFlag = None, printRootPkg: Boolean = false) = {
    render(tree, newCodePrinter(_, tree, printRootPkg), printTypes, printIds, printOwners, printKinds = None,
      printMirrors = None, printPositions)
  }

  protected def render(what: Any, mkPrinter: PrintWriter => TreePrinter, printTypes: BooleanFlag = None,
                       printIds: BooleanFlag = None, printOwners: BooleanFlag = None, printKinds: BooleanFlag = None,
                       printMirrors: BooleanFlag = None, printPositions: BooleanFlag = None): String = {
